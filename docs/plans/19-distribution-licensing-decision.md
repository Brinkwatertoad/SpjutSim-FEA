...
- Do not publicly distribute the combined Gmsh artifact under an all-rights-reserved root license.
...
versions, sources, hashes, modifications, and licenses. At the start of this
task, Gmsh/GMSH-JS was GPL-2.0-or-later, the root granted no compatible
redistribution permission, and the release audit marked public distribution blocked.

## Implementation

- [x] **Obtain the owner decision and review.** Present two release-ready paths
...
- [x] **Implement the approved root-license posture.** For the compatible-source
...
- [x] **Create the distributable manifest.** Add
...
- [x] **Automate compliance checks.** Add `tools/audit-distribution.py` and
...
- [x] **Verify reproducibility.** Rebuild the FEM and Gmsh local runtimes from
...
- [x] **Update user-facing distribution documentation.** Reconcile `README.md`,
...
Implementation evidence: `../release/reproducibility.md`. On 2026-09-06 the
owner approved GPL-2.0-or-later for first-party FEA and copied UI foundation
files. On 2026-09-07 the owner approved the final policy and artifact manifest.
The source-accompanied package passes the staged audit with `--require-approved`;
the licensing row is Pass and Task 19 is complete. Task 20 must still bind
the full acceptance evidence to the candidate.
