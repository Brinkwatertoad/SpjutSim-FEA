(function (root) {
  ...
  function StlImportUI(controller, handlers) {
    ...
    [this.unit,this.angle,this.mode,this.tolerance,this.featureAngle].forEach(function(control){control.onchange=function(){handlers.change();self.updateControls();self.accept.disabled=true;self.status.textContent='Options changed. Review the dimensions and new patches before continuing.';};});
    controller.subscribe(function(){self.render();});
  }
  StlImportUI.prototype.updateControls=function(){this.tolerance.disabled=this.mode.value!=='reconstruct';this.featureAngle.disabled=this.mode.value!=='remesh';document.getElementById('stl-remesh-angle-label').hidden=this.featureAngle.disabled;document.getElementById('stl-reconstruction-tolerance-label').hidden=this.mode.value==='remesh';};
  ...
  StlImportUI.prototype.render=function(){
    ...
    if(geometry.sourceMetadata.remeshing)this.status.textContent=geometry.sourceMetadata.triangleCount+' source triangles; '+geometry.faceIds.length+' selectable patches; '+geometry.sourceMetadata.internalSurfaceCount+' parametrized surfaces. Experimental: generates a fresh surface and volume mesh using your mesh size. Preserves the STL shape as its reference, but does not recover smooth CAD curves. Coarse meshes can lose small details; check the mesh and refine to assess accuracy.';
  };
